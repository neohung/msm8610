...
*/
			if(HWID == 0 || HWID == 1 || HWID == 2){
				//HWID == 1 or HWID == 2, HWID == 3
				//EVT,         DVT1,      DVT2
				printk("HWID: Wi-Fi work around for clock issue (EVT, DVT1, DVT2)\n");
				clk_rf = clk_get(dev, "rf_clk0");
				if (IS_ERR(clk_rf)) {
					pr_err("Couldn't get rf_clk0\n");
					goto fail;
				}
				...
				//HWID == 2 or HWID == 3, HWID == 4
				//DVT3,        PVT,       MP
				printk("HWID: Wi-Fi original code for clock issue (DVT2 and later\n");
				clk_rf = clk_get(dev, "rf_clk1");
				if (IS_ERR(clk_rf)) {
					pr_err("Couldn't get rf_clk1\n");
					goto fail;
				}
