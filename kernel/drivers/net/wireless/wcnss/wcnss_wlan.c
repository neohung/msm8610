...
int wcnss_get_wlan_mac_address(char mac_addr[WLAN_MAC_ADDR_SIZE])
{
 ...
 penv->wlan_nv_macAddr[0]= MAC_address[0]; 
 penv->wlan_nv_macAddr[1]= MAC_address[1]; 
 penv->wlan_nv_macAddr[2]= MAC_address[2]; 
 penv->wlan_nv_macAddr[3]= MAC_address[3]; 
 penv->wlan_nv_macAddr[4]= MAC_address[4]; 
 penv->wlan_nv_macAddr[5]= MAC_address[5]; 

	memcpy(mac_addr, penv->wlan_nv_macAddr, WLAN_MAC_ADDR_SIZE);
	pr_info("%s: Get MAC Addr:" MAC_ADDRESS_STR "\n", __func__,
		penv->wlan_nv_macAddr[0], penv->wlan_nv_macAddr[1],
		penv->wlan_nv_macAddr[2], penv->wlan_nv_macAddr[3],
		penv->wlan_nv_macAddr[4], penv->wlan_nv_macAddr[5]); 
	return 0;
