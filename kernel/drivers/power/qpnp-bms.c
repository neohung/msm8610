...
struct qpnp_bms_chip {
	...
	uint8_t                         qpnp_bms_bat_id_idx;
	bool				qpnp_bms_batt_error;
	struct delayed_work		led_error_ind_delayed_work;
...
static char *qpnp_bms_batt_id_list[] = {
     "Unknown",
     "Highpower",
};
...
#define LED_IND_RED    75
#define LED_IND_GREEN  76
#define HIGHPOWER_BATT_ID_KOHM  10
#define BATT_WARM_TEMP 450    /* 45 degree C */
#define BATT_COOL_TEMP 0      /* 0 degree C */
#define BATT_SOC_RESERVED  5 /* mapping 100~5% to 100%~0% */

static bool is_battery_present(struct qpnp_bms_chip *chip);

static void led_ind_init(void)
{
	gpio_request(LED_IND_RED, "led_ind_red");
	gpio_export(LED_IND_RED, false);
	gpio_direction_output(LED_IND_RED, 0);
	gpio_request(LED_IND_GREEN, "led_ind_green");
	gpio_export(LED_IND_GREEN, false);
	gpio_direction_output(LED_IND_GREEN, 0);
}
static void led_ind_charging(void)
{
	gpio_direction_output(LED_IND_RED, 1);
	gpio_direction_output(LED_IND_GREEN, 1);
}

static void led_ind_full(void)
{
	gpio_direction_output(LED_IND_RED, 0);
	gpio_direction_output(LED_IND_GREEN, 1);
}

static void led_ind_discharging(void)
{
	gpio_direction_output(LED_IND_RED, 0);
	gpio_direction_output(LED_IND_GREEN, 0);
}

static void set_batt_id_idx(struct qpnp_bms_chip *chip, int batt_id)
{
	if(HIGHPOWER_BATT_ID_KOHM == batt_id)
	{
		chip->qpnp_bms_bat_id_idx = 1;
	}
	else
		chip->qpnp_bms_bat_id_idx = 0;
}

static void led_ind_error(struct work_struct *work)
{
	static bool led_red_en = 0;

	struct qpnp_bms_chip *chip = container_of(work,
				struct qpnp_bms_chip,
				led_error_ind_delayed_work.work);

	if(chip->qpnp_bms_batt_error)
	{
	  led_red_en ^= 0x01;
	  gpio_direction_output(LED_IND_RED, led_red_en);
	  gpio_direction_output(LED_IND_GREEN, 0);
          schedule_delayed_work(&chip->led_error_ind_delayed_work, msecs_to_jiffies(150));
	}else{
	  gpio_direction_output(LED_IND_RED, 0);
	  gpio_direction_output(LED_IND_GREEN, 0);
	}
...
static void led_ind_error_condition_chk(struct qpnp_bms_chip *chip, int batt_temp)
{
	int error_condition = 0;

	error_condition = !is_battery_present(chip);

	if(batt_temp >= BATT_WARM_TEMP ||
		batt_temp < BATT_COOL_TEMP)
	{
	  error_condition = 1;
	}

        if(error_condition !=  chip->qpnp_bms_batt_error)
	{
          if(error_condition)
	  {
            schedule_delayed_work(&chip->led_error_ind_delayed_work, msecs_to_jiffies(500));
	  }
	  chip->qpnp_bms_batt_error = error_condition;
	}
}
...
static void led_ind_discharging(void){}
static void set_batt_id_idx(struct qpnp_bms_chip *chip, int batt_id){}
static void led_ind_error(struct qpnp_bms_chip *chip){}
static void led_ind_error_condition_chk(struct qpnp_bms_chip *chip, int batt_temp){}
#endif
...
static int recalculate_soc(struct qpnp_bms_chip *chip)
{
	...
	if (chip->use_voltage_soc) {
		...
		if (rc) {
			...
			led_ind_error_condition_chk(chip, batt_temp);
			mutex_lock(&chip->last_ocv_uv_mutex);
			read_soc_params_raw(chip, &raw, batt_temp);
...
#ifdef CONFIG_MACH_ROADRUNNER
/* Returns manufacturer in string */
static char *get_prop_bms_manufacturer(struct qpnp_bms_chip *chip)
{
	return qpnp_bms_batt_id_list[chip->qpnp_bms_bat_id_idx];
}
#else
static char *get_prop_bms_manufacturer(struct qpnp_bms_chip *chip)
{
	return qpnp_bms_batt_id_list[0];
}
#endif
...
{
	int new_soc = report_state_of_charge(chip);

       if(new_soc >= BATT_SOC_RESERVED)
	  new_soc = 100*(new_soc - BATT_SOC_RESERVED)/(100 - BATT_SOC_RESERVED);
	else
	  new_soc = 0;
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_MANUFACTURER:
		val->strval = get_prop_bms_manufacturer(chip);
		break;
	default:
		return -EINVAL;
	}
...
static int set_battery_data(struct qpnp_bms_chip *chip)
{
	...
	if (chip->batt_type == BATT_DESAY) {
		...
		if (rc == 0 && batt_data->fcc_temp_lut
			...
			dt_data = true;
			set_batt_id_idx(chip, batt_data->batt_id_kohm);
		} else {
...
static int __devinit qpnp_bms_probe(struct spmi_device *spmi)
{
	...
	INIT_DELAYED_WORK(&chip->led_error_ind_delayed_work, led_ind_error);
