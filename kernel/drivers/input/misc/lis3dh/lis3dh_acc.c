...
#define CAL_TOLERANCE		250
#define CAL_X_IDEAL_VALUE	0
#define CAL_Y_IDEAL_VALUE	0
#define CAL_Z_IDEAL_VALUE	1015
...
static void lis3dh_acc_report_values(struct lis3dh_acc_status *stat,
					int *xyz)
{
	input_report_abs(stat->input_dev, ABS_X, xyz[0] - stat->cal_x);
	input_report_abs(stat->input_dev, ABS_Y, xyz[1] - stat->cal_y);
	input_report_abs(stat->input_dev, ABS_Z, xyz[2] - stat->cal_z);
...
static ssize_t attr_get_lis3dh_calibration(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct lis3dh_acc_status *stat = dev_get_drvdata(dev);
	...
	i = counter = 0;
	while (i < 10) {
		ret = lis3dh_acc_get_acceleration_data(stat, xyz);
		if (ret < 0)
			break;
		else {
			x = abs(xyz[0] - CAL_X_IDEAL_VALUE);
			y = abs(xyz[1] - CAL_Y_IDEAL_VALUE);
			z = abs(xyz[2] - CAL_Z_IDEAL_VALUE);

			if (! (x > CAL_TOLERANCE || y > CAL_TOLERANCE || z > CAL_TOLERANCE)) {
				sum_x += xyz[0];
				sum_y += xyz[1];
				sum_z += xyz[2];
				++counter;
	...
	memset(myBuff, 0, 32);
	if (counter == 0)
		sprintf(myBuff, "0,0,0");
	else {
		x = sum_x / counter - CAL_X_IDEAL_VALUE;
		y = sum_y / counter - CAL_Y_IDEAL_VALUE;
		z = sum_z / counter - CAL_Z_IDEAL_VALUE;
		sprintf(myBuff, "%d,%d,%d", x, y, z);
	}

	memcpy(buf, myBuff, 32);
	return 32;
}

static ssize_t attr_set_lis3dh_calibration(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	struct lis3dh_acc_status *stat = dev_get_drvdata(dev);
	int x, y, z;
	int input = 0;

	input = sscanf(buf, "%d,%d,%d", &x, &y, &z);
	if (input == 3 && abs(x) < CAL_TOLERANCE && abs(y) < CAL_TOLERANCE && abs(z) < CAL_TOLERANCE) {
		stat->cal_x = x;
		stat->cal_y = y;
		stat->cal_z = z;
	}
...
static struct device_attribute attributes[] = {
	...
	__ATTR(lis3dh_calibration, 0660, attr_get_lis3dh_calibration, attr_set_lis3dh_calibration),
...
static int lis3dh_acc_input_init(struct lis3dh_acc_status *stat)
{
	...
	stat->input_dev->name = ACCEL_INPUT_DEV_NAME;
...
*/
	...
	stat->cal_x = stat->cal_y = stat->cal_z = 0;
