...
extern s32 gtp_i2c_read_dbl_check(struct i2c_client *client, u16 addr, u8 *rxbuf, int len); // Add by one
extern s32 gup_update_proc(void *dir); // Add by one
extern s8 gtp_enter_sleep(struct goodix_ts_data *ts); // Add by one
extern s8 gtp_wakeup_sleep(struct goodix_ts_data *ts); // Add by one
u8 gtp_get_sensor_id(void); // Add by one
...
static struct gt9xx_test_parameter test_parameter[6] = GTP_BINCFG_GROUP; // Add by one 
struct gt9xx_test_result test_gtresult;
u8 gtp_test_sensor_id; // Add by one
...
static int gt9xx_opentest_max, gt9xx_opentest_min, gt9xx_opentest_uni;

static int gt9xx_uniformity = 100;
...
s32 gtp_parse_config(struct i2c_client *client)
{
	...
	if (!have_key) {
		gt9xx_sc_drv_num = gt9xx_drv_num;
		GTP_INFO("No key");
		return SUCCESS;
	}
	...
	while (left > 0) {
		...
		for (i = 0; i < opr_len; ++i) {
			if (opr_buf[i+2] != dsp_short[addr-GTP_REG_DSP_SHORT + i]) {
				...
				GTP_DEBUG("(%d)Location: %d, 0x%02X, 0x%02X",
						retry+1, addr - GTP_REG_DSP_SHORT + i, opr_buf[i+2], dsp_short[addr-GTP_REG_DSP_SHORT+i]);
...
s32 gt9xx_short_test(struct i2c_client * client)
{
	...
	// Add by one
	test_gtresult.short_test_flag = TRUE;
	test_gtresult.short_test_result = FAIL;
...
load_dsp_again:
	...
	*/
	...
	if ((opr_buf[2] & 0x0f) == 0) {
		SET_INFO_LINE_INFO("B72 short test PASS!");
		ret = SUCCESS;

		// Add by one
		test_gtresult.short_test_result = SUCCESS;
	} else {
		ret2 = SUCCESS;
		...
		if (ret == SUCCESS && ret2 == SUCCESS) {
			...
			SET_INFO_LINE_ERR("B72 short test FAIL!");
			ret = FAIL;
		}
	}
...
static void gtp_raw_min_test(u16 *raw_buf)
{
	...
	{  
		if (raw_buf[i] < test_parameter[gtp_test_sensor_id].min_limit_value)
		{
			...
			if (new_flag)   // new one
			{
				...
				GTP_DEBUG("[%d, %d]rawdata: %d, raw min limit: %d", driver, sensor, raw_buf[i], test_parameter[gtp_test_sensor_id].min_limit_value);
			}
			else
			{
				continue;
...
static void gtp_raw_max_test(u16 *raw_buf)
{
	...
	for (i = 0; i < gt9xx_sc_pxl_cnt; i++)
	{
		if (raw_buf[i] > test_parameter[gtp_test_sensor_id].max_limit_value)
		{
			...
			if (new_flag)   // new one
			{
				...
				touchpad_sum[j].raw_val = raw_buf[i];
				touchpad_sum[j].times = 1;	
				GTP_DEBUG("[%d, %d]rawdata: %d, raw max limit: %d", driver, sensor, raw_buf[i], test_parameter[gtp_test_sensor_id].max_limit_value);
			}
			else
			{
				continue;
...
static void gtp_key_max_test(u16 *raw_buf)
{
	...
	for (i = gt9xx_sc_pxl_cnt; i < gt9xx_pixel_cnt; ++i)
	{
		...
		if (raw_buf[i] > test_parameter[gtp_test_sensor_id].max_limit_key)
		{
			...
			if (new_flag)   // new one
			{
				...
				GTP_DEBUG("[%d, %d]key rawdata: %d, key max limit: %d", driver,sensor, raw_buf[i], test_parameter[gtp_test_sensor_id].max_limit_key);
			}
			else
			{
				continue;
...
static void gtp_key_min_test(u16 *raw_buf)
{
	...
	for (i = gt9xx_sc_pxl_cnt; i < gt9xx_pixel_cnt; ++i)
	{
		...
		if (raw_buf[i] < test_parameter[gtp_test_sensor_id].min_limit_key)
		{
			...
			if (new_flag)   // new one
			{
				...
				GTP_DEBUG("[%d, %d]key rawdata: %d, key min limit: %d", driver, sensor, raw_buf[i], test_parameter[gtp_test_sensor_id].min_limit_key);
...
static void gtp_uniformity_test(u16 *raw_buf)
{
	...
	if (0 == max_val)
		...
		if (gt9xx_uniformity > uniformity)
			gt9xx_uniformity = uniformity;
	}

	GTP_DEBUG("min_val: %d, max_val: %d, tp uniformity: %d%%", min_val, max_val, uniformity);

	if (uniformity < test_parameter[gtp_test_sensor_id].uniformity_lmt) {
...
static void gtp_raw_print(u16 *raw_buf)
{
...
#if PRINT_RAW_DATA 
	SET_INFO_LINE_INFO("Total %d node", gt9xx_sc_pxl_cnt);
#endif

	// Print raw data
	min_val = raw_buf[0];
	max_val = raw_buf[0];
	for (i = 0; i < gt9xx_sc_drv_num; i++) { // sc_srv_num alread sub key driver line

#if PRINT_RAW_DATA
		SET_INFO_SHORT_INFO("D%02d:", i);
#endif

		for (j = 0; j < gt9xx_sen_num; ++j) {
			ds_val = (i * gt9xx_sen_num) + j; // Target d/s rawdata buf value
			(raw_buf[ds_val] > max_val) ? (max_val = raw_buf[ds_val]) : 0; // Get max raw data
			(raw_buf[ds_val] < min_val) ? (min_val = raw_buf[ds_val]) : 0; // Get min raw data
	   		avg_val += raw_buf[ds_val];

#if PRINT_RAW_DATA
			SET_INFO_SHORT_INFO(" %d", raw_buf[ds_val]);
#endif
	...
	SET_INFO_LINE_INFO("Test Limit Max:%d, Min:%d",
			test_parameter[gtp_test_sensor_id].max_limit_value, test_parameter[gtp_test_sensor_id].min_limit_value);
	SET_INFO_LINE_INFO("TP Max:%d, Min:%d, Avg:%d",
			max_val, min_val, (avg_val / gt9xx_sc_pxl_cnt));
}
...
static s32 gtp_get_test_result(void)
{
	...
	if (beyond_max_num > _MIN_ERROR_NUM) {
		...
		j = 0;
		SET_INFO_LINE_ERR("Beyond Max:");

		for (i = 1; i <= _BEYOND_INFO_MAX; ++i) {
			if (touchpad_sum[i].driver == 0xFF)
				break;

			SET_INFO_SHORT_INFO("Drv %d Sen %d: %d",
					touchpad_sum[i].driver, touchpad_sum[i].sensor, touchpad_sum[i].raw_val);
	...
	if (have_key) {
		if (beyond_key_max > _MIN_ERROR_NUM) {
			...
			SET_INFO_LINE_ERR("Beyond Key Max:");

			for (i = 2*_BEYOND_INFO_MAX; i < (3*_BEYOND_INFO_MAX); ++i) {
				if (touchpad_sum[i].driver == 0xFF)
					break;

				SET_INFO_SHORT_INFO("  Drv: %d, Sen: %d RawVal: %d\n",
						touchpad_sum[i].driver, touchpad_sum[i].sensor, touchpad_sum[i].raw_val);
			}
		}
		...
		if (beyond_key_min > _MIN_ERROR_NUM) {
			...
			SET_INFO_LINE_INFO("Beyond Key Min:");
	...
	if (beyond_uniformity > _MIN_ERROR_NUM) {
		...
		SET_INFO_LINE_ERR("Beyond Uniformity:");
		SET_INFO_LINE_ERR("Uniformity Limit: %d%% TP Uniformity: %d%%",
				test_parameter[gtp_test_sensor_id].uniformity_lmt,
				touchpad_sum[4*_BEYOND_INFO_MAX].raw_val / touchpad_sum[4 * _BEYOND_INFO_MAX].times);
	} else
		SET_INFO_LINE_INFO("TP Uniformity: %d%%", gt9xx_uniformity);

	if (result == 0) {
		SET_INFO_LINE_INFO("B72 open test PASS!");

		// Add by one
		test_gtresult.open_test_result = SUCCESS;
		return result;
	}

	SET_INFO_LINE_INFO("B72 open test FAIL!");
	if (result & _BEYOND_MAX_LIMIT)
		SET_INFO_LINE_INFO("Beyond Max [Max Limit: %d]", test_parameter[gtp_test_sensor_id].max_limit_value);
	if (result & _BEYOND_MIN_LIMIT)
		SET_INFO_LINE_INFO("Beyond Min [Min Limit: %d]", test_parameter[gtp_test_sensor_id].min_limit_value);

	if (have_key) {
		if (result & _BEYOND_KEY_MAX_LMT)
			SET_INFO_LINE_INFO("Beyond Key Max [Key Max Limit: %d]", test_parameter[gtp_test_sensor_id].max_limit_key);
		if (result & _BEYOND_KEY_MIN_LMT)
			SET_INFO_LINE_INFO("Beyond Key Min [Key Min Limit: %d]", test_parameter[gtp_test_sensor_id].min_limit_key);
	}

	if (result & _BEYOND_UNIFORMITY_LMT)
		SET_INFO_LINE_INFO("Beyond Uniformity [Uniformity Limit: %d%%]", test_parameter[gtp_test_sensor_id].uniformity_lmt);
...
s32 gt9xx_open_test(struct i2c_client * client)
{
	...
	// Add by one, send cfg
	test_gtresult.open_test_flag = TRUE;
	test_gtresult.open_test_result = FAIL;

	// Add by one to read sensor id and set test parameter
	...
	gtp_raw_print(raw_buf); // Only printf last raw data info
 ...
 *
 ===================================================
*/
u8 gt9xx_bincfg_check(struct i2c_client * client)
{
	u16 i = 0;
	s32 ret = FAIL; // SUCCESS, FAIL
	struct goodix_ts_data *ts;
	u8 buf[10] = { GTP_REG_VERSION >> 8, GTP_REG_VERSION & 0xff };
	u8 ansbuf[10] = GTP_IC_VERSION;
	...
	SET_INFO_LINE_INFO("---gtp bincfg check start---");

	// Add by one, send cfg
	test_gtresult.bincfg_check_flag = TRUE;
	test_gtresult.bincfg_check_result = FAIL;
	ret = gtp_send_cfg(ts->client);
	if (ret < 0)
		GTP_ERROR("Send config error.");
	...
	gtp_test_sensor_id = gtp_get_sensor_id();
	if ((gtp_test_sensor_id > 5) || (gtp_test_sensor_id < 0))
		goto bincfg_chk_exit;

	// Check IC and F/W version
	buf[0] = (GTP_REG_VERSION >> 8), buf[1] = (GTP_REG_VERSION & 0xff);
	gtp_i2c_read(client, buf, sizeof(buf));
	i = (buf[7] << 8) + buf[6];
	if (buf[5] == 0x00) {
		SET_INFO_LINE_INFO("IC_FW Version: %c%c%c_%x(def), %c%c%c_%x(sor)",
				ansbuf[0], ansbuf[1], ansbuf[2], GTP_BIN_VERSION, buf[2], buf[3], buf[4], i);
		ansbuf[3] = 0x00;
	} else {
		SET_INFO_LINE_INFO("IC_FW Version: %c%c%c%c_%x(def), %c%c%c%c_%x(sor)",
				ansbuf[0], ansbuf[1], ansbuf[2], ansbuf[3], GTP_BIN_VERSION, buf[2], buf[3], buf[4], buf[5], i);
	}

	if ((ansbuf[0] != buf[2]) || (ansbuf[1] != buf[3]) || (ansbuf[2] != buf[4]) || (ansbuf[3] != buf[5]) || (i != GTP_BIN_VERSION)) {
		SET_INFO_LINE_ERR("B72 IC or FW version check FAIL!");
		goto bincfg_chk_exit;
	} else
		SET_INFO_LINE_INFO("B72 IC, FW version check PASS!");

	// Check CFG version
	buf[0] = (GTP_REG_CONFIG_DATA >> 8), buf[1] = (GTP_REG_CONFIG_DATA & 0xff);
	gtp_i2c_read(client, buf, sizeof(buf));
	SET_INFO_LINE_INFO("CFG Version: 0x%X(def), 0x%X(sor)", test_parameter[gtp_test_sensor_id].cfg_version, buf[2]);

	if (buf[2]!= test_parameter[gtp_test_sensor_id].cfg_version) {
		SET_INFO_LINE_ERR("B72 CFG version check FAIL!");
		goto bincfg_chk_exit;
	}
	SET_INFO_LINE_INFO("B72 CFG version check PASS!");

	// Check chksum first byte
	buf[0] = (0x80ff >> 8), buf[1] = (0x80ff & 0xff);
	gtp_i2c_read(client, buf, sizeof(buf));

	// Check chksum sencond byte
	SET_INFO_LINE_INFO("0x80FF buf[2] = 0x%X", buf[2]);
	if (buf[2] != test_parameter[gtp_test_sensor_id].cfg_chksum) {
		buf[0] = (0x8129 >> 8), buf[1] = (0x8129 & 0xff);
		gtp_i2c_read(client, buf, sizeof(buf));
		SET_INFO_LINE_INFO("0x8129 buf[2] = 0x%X", buf[2]);
		if (buf[2] != test_parameter[gtp_test_sensor_id].cfg_chksum) {
			SET_INFO_LINE_ERR("B72 CFG Chksum check FAIL!");
			goto bincfg_chk_exit;
		}
	}

	SET_INFO_LINE_INFO("CFG Chksum: 0x%02X(def), 0x%02X(sor)", test_parameter[gtp_test_sensor_id].cfg_chksum, buf[2]);
	SET_INFO_LINE_INFO("B72 CFG Chksum check PASS!");

	test_gtresult.bincfg_check_result = SUCCESS;

	SET_INFO_LINE_INFO("---gtp bincfg check end---");
...
bincfg_chk_exit:
	...
	return test_gtresult.bincfg_check_result;
}
 ...
 *
 ===================================================
*/
s32 gt9xx_rstint_i2c_test(struct i2c_client * client)
{
	u16 i;
	struct goodix_ts_data *ts;
	...
	// Add by one, send cfg
	test_gtresult.rstint_i2c_test_flag = TRUE;
	test_gtresult.rstint_i2c_test_result = FAIL;
	...
	SET_INFO_LINE_INFO("---gtp rst int i2c start---");

	// Test I2C 50 cycle
	gtp_reset_guitar(ts->client, 10); // Add by one, before I2C test reset IC for address select
	SET_INFO_LINE_INFO("I2C address = 0x%x", ts->client->addr);
	for( i = 0; i < 50 ; i ++) {
		if (!gtp_i2c_test_check(ts->client)) {
			SET_INFO_LINE_ERR("B72 I2C test FAIL!");
			goto rstint_i2c_test_exit;
		}
	}
	SET_INFO_LINE_INFO("B72 I2C test PASS!");

	// Test RST, hold RST can't read I2C information, if read any i2c data then RST isn't connect
	GTP_GPIO_OUTPUT(GTP_RST_PORT, 0); // Reset ic
	msleep(10);
	if (gtp_i2c_test_check(ts->client)) {
		SET_INFO_LINE_ERR("B72 RST test FAIL!");
		goto rstint_i2c_test_exit;
	}
	SET_INFO_LINE_INFO("B72 RST test PASS!");

	// Test INT, if address = 0x5d, the select 0x14 can't read i2c information
	(ts->client->addr == 0x14) ? (ts->client->addr = 0x5d) : (ts->client->addr = 0x14); // Exchange I2C address
	gtp_reset_guitar(ts->client, 10);

	(ts->client->addr == 0x14) ? (ts->client->addr = 0x5d) : (ts->client->addr = 0x14); // Exchange I2C address
	if (gtp_i2c_test_check(ts->client)) {
		SET_INFO_LINE_ERR("B72 INT test FAIL!");
		goto rstint_i2c_test_exit;
	}
	SET_INFO_LINE_INFO("B72 INT test PASS!");
	gtp_reset_guitar(ts->client, 10);

	// Before leave test i2c again
	if (!gtp_i2c_test_check(ts->client)) {
		SET_INFO_LINE_ERR("B72 I2C test FAIL!");
		goto rstint_i2c_test_exit;
	}

	test_gtresult.rstint_i2c_test_result = SUCCESS;

	SET_INFO_LINE_INFO("---gtp rst int i2c test end---");

rstint_i2c_test_exit:
	enable_irq(ts->client->irq);

#if GTP_ESD_PROTECT
	ts->gtp_is_suspend = 0; // Resume ESD
#endif
	return test_gtresult.rstint_i2c_test_result;
}
...
u8 gtp_get_sensor_id(void)
{
	s32 ret = -1;
		...
		GTP_INFO("Sensor_ID: %d", gtp_test_sensor_id);

		return gtp_test_sensor_id;
			
}

static void gtp_test_gtresult_check(void)
{
	//check all test finish
	if ((test_gtresult.rstint_i2c_test_flag == TRUE) && (test_gtresult.bincfg_check_flag == TRUE) &&
		(test_gtresult.open_test_flag == TRUE) && (test_gtresult.short_test_flag == TRUE))
	{
		//print one result for all test 
 		if ((test_gtresult.rstint_i2c_test_result == SUCCESS) && (test_gtresult.bincfg_check_result == SUCCESS) &&
		(test_gtresult.open_test_result == SUCCESS) && (test_gtresult.short_test_result == SUCCESS))	
		{
				SET_INFO_LINE_INFO("\n\n\n------------ All test PASS! --------------");		
		}
		else
		{
				SET_INFO_LINE_ERR("\n\n\n********** Test FAIL ************, %d-%d-%d-%d", test_gtresult.rstint_i2c_test_result, test_gtresult.bincfg_check_result, test_gtresult.open_test_result, test_gtresult.short_test_result);
		}	
		//clear all test result
		memset(&test_gtresult, 0, sizeof(test_gtresult)); 
	}
}
...
static ssize_t gtp_sysfs_shorttest_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	gtp_test_gtresult_check();//if each test finish. then show one result
...
// Add by one
static ssize_t gtp_sysfs_bincfg_check_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	s32 index;
	ssize_t len;
	
	gt9xx_bincfg_check(i2c_connect_client);
	gtp_test_gtresult_check();//if each test finish. then show one result
...
static ssize_t gtp_sysfs_rstint_i2c_test_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	s32 index;
	ssize_t len;
	
	gt9xx_rstint_i2c_test(i2c_connect_client);
	gtp_test_gtresult_check();//if each test finish. then show one result
...
// Add by one
static ssize_t gtp_sysfs_bincfg_check_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	return -EPERM;
}

static ssize_t gtp_sysfs_rstint_i2c_test_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	return -EPERM;
}
...
// Add by one
static DEVICE_ATTR(bincfg_check, S_IRUGO|S_IWUSR, gtp_sysfs_bincfg_check_show, gtp_sysfs_bincfg_check_store);
static DEVICE_ATTR(rstint_i2c_test, S_IRUGO|S_IWUSR, gtp_sysfs_rstint_i2c_test_show, gtp_sysfs_rstint_i2c_test_store);
...
static ssize_t gtp_sysfs_open_max_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	char myBuff[16];
	memset(myBuff, 0, 16);

	sprintf(myBuff, "%d", gt9xx_opentest_max);
	memcpy(buf, myBuff, 16);

	return 16;
}

static ssize_t gtp_sysfs_open_max_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int myMax = 0;

	sscanf(buf, "%d", &myMax);
	gt9xx_opentest_max = myMax;

	gtp_test_sensor_id = gtp_get_sensor_id();
	if ((gtp_test_sensor_id > 5) || (gtp_test_sensor_id < 0))
		gtp_test_sensor_id = 0;

	test_parameter[gtp_test_sensor_id].max_limit_value = gt9xx_opentest_max;

	return count;
}

static ssize_t gtp_sysfs_open_min_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	char myBuff[16];
	memset(myBuff, 0, 16);

	sprintf(myBuff, "%d", gt9xx_opentest_min);
	memcpy(buf, myBuff, 16);

	return 16;
}

static ssize_t gtp_sysfs_open_min_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int myMin = 0;

	sscanf(buf, "%d", &myMin);
	gt9xx_opentest_min = myMin;

	gtp_test_sensor_id = gtp_get_sensor_id();
	if ((gtp_test_sensor_id > 5) || (gtp_test_sensor_id < 0))
		gtp_test_sensor_id = 0;

	test_parameter[gtp_test_sensor_id].min_limit_value = gt9xx_opentest_min;

	return count;
}

static DEVICE_ATTR(openmax, 0644, gtp_sysfs_open_max_show, gtp_sysfs_open_max_store);
static DEVICE_ATTR(openmin, 0644, gtp_sysfs_open_min_show, gtp_sysfs_open_min_store);

static ssize_t gtp_sysfs_uniformity_min_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	char myBuff[16];
	memset(myBuff, 0, 16);

	sprintf(myBuff, "%d", gt9xx_opentest_uni);
	memcpy(buf, myBuff, 16);

	return 16;
}

static ssize_t gtp_sysfs_uniformity_min_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int myMin = 0;

	sscanf(buf, "%d", &myMin);
	gt9xx_opentest_uni = myMin;

	gtp_test_sensor_id = gtp_get_sensor_id();
	if ((gtp_test_sensor_id > 5) || (gtp_test_sensor_id < 0))
		gtp_test_sensor_id = 0;

	test_parameter[gtp_test_sensor_id].uniformity_lmt = gt9xx_opentest_uni;

	return count;
}

static DEVICE_ATTR(uniformity_min, 0644, gtp_sysfs_uniformity_min_show, gtp_sysfs_uniformity_min_store);
...
s32 gtp_test_sysfs_init(void)
{
	...
	// Add by one, for test F/W and CFG version and auto-update if firmware.h have define.
	ret = sysfs_create_file(goodix_debug_kobj, &dev_attr_bincfg_check.attr);
	if (ret) {
		GTP_ERROR("%s: sysfs_create_version_file failed\n", __func__);
		return ret;
	}

	// Add by one, for test auto-slect I2C address(RST/INT pin) and I2C test
	ret = sysfs_create_file(goodix_debug_kobj, &dev_attr_rstint_i2c_test.attr);
	if (ret) {
		GTP_ERROR("%s: sysfs_create_version_file failed\n", __func__);
		return ret;
	}

	// Add by one, touch panle disable for digitizer test
	...
	// Add by one, touch panle enable for digitizer test
	...
	ret = sysfs_create_file(goodix_debug_kobj, &dev_attr_openmax.attr);
	if (ret) {
		GTP_ERROR("%s: sysfs_create_file failed\n", __func__);
		return ret;
	}

	ret = sysfs_create_file(goodix_debug_kobj, &dev_attr_openmin.attr);
	if (ret) {
		GTP_ERROR("%s: sysfs_create_file failed\n", __func__);
		return ret;
	}

	ret = sysfs_create_file(goodix_debug_kobj, &dev_attr_uniformity_min.attr);
	if (ret) {
		GTP_ERROR("%s: sysfs_create_file failed\n", __func__);
		return ret;
	}
	...
	// Set test limits
	gt9xx_opentest_max = GT9XX_OPENTEST_MAX;
	gt9xx_opentest_min = GT9XX_OPENTEST_MIN;
	gt9xx_opentest_uni = GT9XX_OPENTEST_UNI;
	test_parameter[gtp_test_sensor_id].max_limit_value = gt9xx_opentest_max;
	test_parameter[gtp_test_sensor_id].min_limit_value = gt9xx_opentest_min;
	test_parameter[gtp_test_sensor_id].uniformity_lmt = gt9xx_opentest_uni;
...
void gtp_test_sysfs_deinit(void)
{
	...
	// Add by one
	sysfs_remove_file(goodix_debug_kobj, &dev_attr_bincfg_check.attr);
	sysfs_remove_file(goodix_debug_kobj, &dev_attr_rstint_i2c_test.attr);
	sysfs_remove_file(goodix_debug_kobj, &dev_attr_tp_disable.attr);
	sysfs_remove_file(goodix_debug_kobj, &dev_attr_tp_enable.attr);
