...
static bool gtp_device_ready = false;
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	GTP_INFO("Driver Built@%s, %s", __TIME__, __DATE__);
...
static int gtp_do_suspend(struct goodix_ts_data *ts)
{
	int ret = 0, retry = 0;
	u8 i2c_control_buf[3] = { (u8)(GTP_REG_SLEEP >> 8), (u8)GTP_REG_SLEEP, 5 };
	...
	while (retry++ < 5) {
		ret = gtp_i2c_write(ts->client, i2c_control_buf, 3);
		if (ret > 0) {
			GTP_INFO("GTP enter sleep success");
			return ret;
		}
		msleep(10);
	}

	GTP_ERROR("Failed to enter sleep");
	return ret;
}

static int gtp_do_resume(struct goodix_ts_data *ts)
{
	int ret = 0, retry = 0;
	...
	while (retry++ < 10) {
		...
		ret = gtp_i2c_test(ts->client);
		if (ret > 0) {
			GTP_INFO("GTP do resume success");
			gtp_int_sync(25);
			return ret;
		}
...
static int goodix_ts_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	if (! ts->gtp_is_suspend) {
		...
		ret = gtp_do_suspend(ts);
		if (ret < 0)
			GTP_ERROR("Failed to enter suspend");
		else {
			// To avoid waking up while not sleeping
			// Delay 48 + 10ms to ensure reliability
			msleep(58);
	...
	mutex_unlock(&ts->lock_suspend);

	GTP_INFO("Suspend success");
	return ret;
}
...
static int goodix_ts_resume(struct i2c_client *client)
{
	...
	mutex_lock(&ts->lock_suspend);
	if (ts->gtp_is_suspend) {
		ret = gtp_do_resume(ts);
		if (ret < 0)
			GTP_ERROR("Failed to do resume");
		else {
			gtp_send_cfg(ts->client);
...
void extern_goodix_ts_suspend(void)
{
	...
	if (gtp_device_ready)
		goodix_ts_suspend(i2c_connect_client, msg);
#endif
}
EXPORT_SYMBOL(extern_goodix_ts_suspend);

void extern_goodix_ts_resume(void)
{
#ifndef CONFIG_HAS_EARLYSUSPEND
	if (gtp_device_ready)
		goodix_ts_resume(i2c_connect_client);
#endif
}
EXPORT_SYMBOL(extern_goodix_ts_resume);
...
static int __devinit goodix_ts_init(void)
{
	...
	GTP_INFO("GTP driver installing...end:%d",ret);
