...
#define PRINT_RAW_DATA			1 // Output log for all node rawdata value

// add by one for BIN/CFG test
#define GTP_IC_VERSION			"9271" // "915", "968", "927", "928", ...
#define GTP_BIN_VERSION			0x1020 // The same with debug board

#define GT9XX_OPENTEST_MAX		2800
#define GT9XX_OPENTEST_MIN		1400
#define GT9XX_OPENTEST_UNI		70

// Sensor_ID == 0 config here.
// Sensor_ID == 1 config here.
// Sensor_ID == 2 config here.
// Sensor_ID == 3 config here.
// Sensor_ID == 4 config here.
// Sensor_ID == 5 config here.
// CFG_version, CFG_Chksum, max_limit, min_limit, key_val max limit, key_value min limit, screen uniformity in percent
#define GTP_BINCFG_GROUP		{							\
						{ 0x42, 0x7B, 8000, 100, 8000, 100, 70 },	\
						{ 72, 0xa6, 3150, 2050, 2800, 1400, 75 },	\
						{ 60, 0x01, 8000, 100, 8000, 100, 70 },		\
						{ 60, 0x01, 8000, 100, 8000, 100, 70 },		\
						{ 60, 0x01, 8000, 100, 8000, 100, 70 },		\
						{ 60, 0x01, 8000, 100, 8000, 100, 70 },		\
					}
...
#define GTP_TEST_INFO_MAX		1460 // test info lines max count. org = 200 + s*d=1260, max 1460
...
//add by one
struct gt9xx_test_parameter
{
	u8 cfg_version; // 0x804f cfg version
	u8 cfg_chksum; // 0x80ff cfg check sum
	u16 max_limit_value; // screen max limit
	u16 min_limit_value; // screen min limit
	u16 max_limit_key; // key_val max limit
	u16 min_limit_key; // key_val min limit
	u16 uniformity_lmt; // screen uniformity in percent
};

struct gt9xx_test_result
{
	u8 open_test_flag;
	u8 open_test_result;

	u8 short_test_flag;
	u8 short_test_result;

	u8 bincfg_check_flag;
	u8 bincfg_check_result;

	u8 rstint_i2c_test_flag;
	u8 rstint_i2c_test_result;
};

#define SET_INFO_SHORT_INFO(fmt, args...)	do {								\
